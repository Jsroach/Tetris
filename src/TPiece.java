 ...
 * 
 * @author Jack Roach
 * subclass of Piece that creates the T shaped tetris piece
 */
public class TPiece extends Piece{
	...
	public TPiece(TetrisBoard board) {
		...
		squares[1].moveToTetrisLocation(TetrisBoard.X_DIM_SQUARES/2 - 1, 3);
	...
	public void rotateRight(TetrisBoard board) {
		if (squares[0].getY() == TetrisBoard.Y_DIM_SQUARES-1 && squares[3].getY() < squares[0].getY()){
			if (board.isOccupied(squares[0].getX(), squares[0].getY()-1) == false &&
				board.isOccupied(squares[1].getX(), squares[1].getY()-1) == false &&
				board.isOccupied(squares[2].getX(), squares[2].getY()-1) == false &&
				board.isOccupied(squares[3].getX(), squares[3].getY()-1) == false){
	...
	/**
	 * rotates all 4 squares 90 degrees counterclockwise where there is room
	 */
